...
public class UploadServletAPI extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		Long streamId = tstImage.streamId;
		Stream st = ofy().load().type(Stream.class).id(streamId).get();
		String streamName = st.getName();
		String content = "uploaded from app";		
		st.appendTags(tstImage.tags);
		st.setDate();
		ofy().save().entity(st).now();
		ConnexusImage s = new ConnexusImage(streamId, streamName, content, imageUrl);
		s.setLocation(tstImage.latitude, tstImage.longitude);
